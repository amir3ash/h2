...
// might be 0).
type frameParser func(fc *frameCache, fh FrameHeader, countError func(string), payload []byte) (frameInvalidate, error)
...
func (h FrameHeader) Header() http2.FrameHeader { return h.FrameHeader }
...
func (h FrameHeader) writeDebug(buf *bytes.Buffer) {
	...
	if h.Flags != 0 {
		...
		for i := uint8(0); i < 8; i++ {
			...
			name := flagName[h.Type][http2.Flags(1<<i)]
			if name != "" {
...
type frameInvalidate interface{
	Frame
	invalidate()
}
...
func (fr *Framer) SetWriter(w *bytes.Buffer) {
	fr.w = w
}
...
func (f *Framer) startWrite(ftype http2.FrameType, flags http2.Flags, streamID uint32) {
...
func (f *Framer) endWrite() error {
	...
	length := f.w.Len() - frameHeaderLen
	if length >= (1 << 24) {
		return ErrFrameTooLarge
	}
	_ = append(f.w.Bytes()[:0],
		byte(length>>16),
		byte(length>>8),
		byte(length))
	if f.logWrites {
...
func (f *Framer) logWrite() {
	if f.debugFramer == nil {
		...
		f.debugFramer = NewFramer()
		f.debugFramer.SetWriter(f.debugFramerBuf)
...
func (f *Framer) writeByte(v byte)     { f.w.WriteByte(v) }
func (f *Framer) writeBytes(v []byte)  { f.w.Write(v) }
func (f *Framer) writeUint16(v uint16) { f.w.Write([]byte{byte(v >> 8), byte(v)}) }
func (f *Framer) writeUint32(v uint32) {
	f.w.Write([]byte{byte(v >> 24), byte(v >> 16), byte(v >> 8), byte(v)})
}
...
func parseDataFrame(fc *frameCache, fh FrameHeader, countError func(string), payload []byte) (frameInvalidate, error) {
	if fh.StreamID == 0 {
...
func (f *Framer) startWriteDataPadded(streamID uint32, endStream bool, data, pad []byte) error {
	...
	f.startWrite(http2.FrameData, flags, streamID)
	if pad != nil {
		f.w.WriteByte(byte(len(pad)))
	}
	f.w.Write(data)
	f.w.Write(pad)
	return nil
...
func parseSettingsFrame(_ *frameCache, fh FrameHeader, countError func(string), p []byte) (frameInvalidate, error) {
...
func (f *SettingsFrame) HasDuplicates() bool {
	...
	seen := map[http2.SettingID]struct{}{}
	for i := 0; i < num; i++ {
		id := f.Setting(i).ID
		if _, ok := seen[id]; ok {
			return true
...
func (f *Framer) WriteSettings(settings ...http2.Setting) error {
	f.startWrite(http2.FrameSettings, 0, 0)
...
func (f *Framer) WriteSettingsAck() error {
	f.startWrite(http2.FrameSettings, http2.FlagSettingsAck, 0)
	return f.endWrite()
}
...
func (f *PingFrame) IsAck() bool { return f.Flags.Has(http2.FlagPingAck) }

func parsePingFrame(_ *frameCache, fh FrameHeader, countError func(string), payload []byte) (frameInvalidate, error) {
	if len(payload) != 8 {
...
func (f *Framer) WritePing(ack bool, data [8]byte) error {
	...
	f.startWrite(http2.FramePing, flags, 0)
	f.writeBytes(data[:])
	return f.endWrite()
}
...
func (f *Framer) WriteGoAway(maxStreamID uint32, code http2.ErrCode, debugData []byte) error {
	f.startWrite(http2.FrameGoAway, 0, 0)
...
func parseUnknownFrame(_ *frameCache, fh FrameHeader, countError func(string), p []byte) (frameInvalidate, error) {
...
func parseWindowUpdateFrame(_ *frameCache, fh FrameHeader, countError func(string), p []byte) (frameInvalidate, error) {
	if len(p) != 4 {
...
func (f *Framer) WriteWindowUpdate(streamID, incr uint32) error {
	...
	f.startWrite(http2.FrameWindowUpdate, 0, streamID)
	f.writeUint32(incr)
	return f.endWrite()
}
...
func parseHeadersFrame(_ *frameCache, fh FrameHeader, countError func(string), p []byte) (_ frameInvalidate, err error) {
	hf := &HeadersFrame{
		FrameHeader: fh,
	}
	if fh.StreamID == 0 {
...
func (f *Framer) WriteHeaders(p http2.HeadersFrameParam) error {
	...
	f.startWrite(http2.FrameHeaders, flags, p.StreamID)
	if p.PadLength != 0 {
	...
	f.w.Write(p.BlockFragment)
	f.w.Write(padZeros[:p.PadLength])
	return f.endWrite()
}
...
func parsePriorityFrame(_ *frameCache, fh FrameHeader, countError func(string), payload []byte) (frameInvalidate, error) {
	if fh.StreamID == 0 {
...
func (f *Framer) WritePriority(streamID uint32, p PriorityParam) error {
	...
	f.startWrite(http2.FramePriority, 0, streamID)
	v := p.StreamDep
	if p.Exclusive {
		v |= 1 << 31
	}
...
func parseRSTStreamFrame(_ *frameCache, fh FrameHeader, countError func(string), p []byte) (frameInvalidate, error) {
	if len(p) != 4 {
	...
	return &RSTStreamFrame{fh, http2.ErrCode(binary.BigEndian.Uint32(p[:4]))}, nil
}
...
func (f *Framer) WriteRSTStream(streamID uint32, code http2.ErrCode) error {
	...
	f.startWrite(http2.FrameRSTStream, 0, streamID)
	f.writeUint32(uint32(code))
	return f.endWrite()
}
...
func parseContinuationFrame(_ *frameCache, fh FrameHeader, countError func(string), p []byte) (frameInvalidate, error) {
	if fh.StreamID == 0 {
...
func (f *Framer) WriteContinuation(streamID uint32, endHeaders bool, headerBlockFragment []byte) error {
	...
	f.startWrite(http2.FrameContinuation, flags, streamID)
	f.w.Write(headerBlockFragment)
	return f.endWrite()
}
...
func parsePushPromise(_ *frameCache, fh FrameHeader, countError func(string), p []byte) (_ frameInvalidate, err error) {
	pp := &PushPromiseFrame{
		FrameHeader: fh,
	}
	if pp.StreamID == 0 {
...
func (f *Framer) WritePushPromise(p PushPromiseParam) error {
	...
	f.startWrite(http2.FramePushPromise, flags, p.StreamID)
	if p.PadLength != 0 {
		f.writeByte(p.PadLength)
	}
